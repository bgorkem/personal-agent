...
Every post ships working code. The repo is public, and each post has a git tag so you can `git checkout post-01-hello-agent` and play.

---

## First, a confusion to clear up

When I tell people I'm building an agent with "Claude", they assume one of two things:

1. I'm using Claude Code, the CLI coding tool
2. I'm calling the Anthropic Messages API directly

Neither is quite right. I'm using the **Claude Agent SDK**, which is a distinct third thing. The difference matters because most "build an agent" tutorials online either skip past it or conflate it with one of the other two.

Here's the actual landscape:

| Thing | What it is | When to use it |
|---|---|---|
| **Anthropic API** | The raw `/v1/messages` HTTP endpoint | Maximum control. You build the agent loop yourself. |
| **Claude Code** | A standalone CLI that *uses* the SDK to edit your codebase | You want a coding assistant, not to build one. |
| **Claude Agent SDK** | An npm package (`@anthropic-ai/claude-agent-sdk`) | You're building your own agent and want the loop, tools, and context handling done for you. |

That third row is us. The SDK gives us the same runtime that powers Claude Code, but as a library we can `import` into our own programs.
...
3. if tool: run it, send result back to model, GOTO 2
...
The reason agents feel magical is that step 2 happens *autonomously*. The model decides on its own which tool to call, when, with what arguments, and whether to call another one based on the result. Your job as the developer is to:

- Give it a good set of tools
- Give it enough context to use them well
- Stay out of its way
...
- **A framework.** Hono, Fastify, NestJS — none of them. The whole project is `node --run start`. We'll add a transport later when there's something to transport to.
...
Code for this post is tagged `post-01-hello-agent` in the repo. See you in the next one.
